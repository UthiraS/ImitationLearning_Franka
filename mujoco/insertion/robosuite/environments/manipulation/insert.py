...
class Insert(SingleArmEnv):
    ...
    def __init__(
        ...
        self.data_history={'peg_pos':[], 'hole_pos':[], 'force':[], 'torque':[]}
        #Bounds for plots
        self.fxy_minmax=[-0.25,0.25]
        self.fz_minmax=[-5.25,-4.75]
        self.T_minmax=[-0.1,0.1]
        self.episode_num=1
        now=datetime.now()
        self.time_id=now.strftime("%m%d%Y_%H%M")
    ...
    def _post_action(self, action):
        ...
        if self.save_obs_data:
            self.data_history['peg_pos'].append([self.sim.data.body_xpos[self.peg_body_id][0],
                                                 self.sim.data.body_xpos[self.peg_body_id][1],
                                                 self.sim.data.body_xpos[self.peg_body_id][2]]) #3 XYZ
            self.data_history['hole_pos'].append([self.sim.data.body_xpos[self.hole_body_id][0],
                                                  self.sim.data.body_xpos[self.hole_body_id][1],
                                                  self.sim.data.body_xpos[self.hole_body_id][2]]) #3 XYZ
            self.data_history['force'].append(self.robots[0].ee_force) #3 fx,fy,fz
            self.data_history['torque'].append(self.robots[0].ee_torque) #3 tx,ty,tz
        if self.live_plot:
            if self.timestep==1:
                #Plot for peg position
                plt.figure(1)
                plt.axis([-0.015,0.015,0.015,-0.015])
                plt.grid()
                plt.xlabel('Y (m)')
                plt.ylabel('X (m)')
                plt.title('Peg and Hole Positions')
                #Plot for F_xy Readings
                plt.figure(2)
                plt.subplot(311)
                
                plt.axis([0.,self.horizon/4,self.fxy_minmax[0],self.fxy_minmax[1]])
                plt.grid()
                #plt.xlabel('Timestep (-)')
                plt.ylabel('Force (N)')
                plt.title('EE Wrist Force (X&Y)') 
                plt.subplots_adjust(hspace=0.9)
                #Plot for F_z Readings
                plt.subplot(312)
                
                plt.axis([0.,self.horizon/4,self.fz_minmax[0],self.fz_minmax[1]])
                plt.grid()
                #plt.xlabel('Timestep (-)')
                plt.ylabel('Force (N)')
                plt.title('EE Wrist Force (Z)') 
                plt.subplots_adjust(hspace=0.9)
                #Plot for T Readings
                plt.subplot(313)
                
                plt.axis([0.,self.horizon/4,self.T_minmax[0],self.T_minmax[1]])
                plt.grid()
                plt.xlabel('Timestep (-)')
                plt.ylabel('Torque (Nm)')
                plt.title('EE Wrist Torque') 
                plt.subplots_adjust(hspace=0.9)
            #Add new data to plots
            current_peg_pos = self.sim.data.body_xpos[self.peg_body_id]
            current_hole_pos = self.sim.data.body_xpos[self.hole_body_id]
            plt.figure(1)
            plt.scatter(current_peg_pos[1], current_peg_pos[0], 
                        marker='o', s=2, c='red', label='Peg')
            plt.scatter(current_hole_pos[1], current_hole_pos[0], 
                        marker='o', s=2, c='blue', label='Hole')
            plt.figure(2)
            plt.subplot(311)
            plt.scatter(self.timestep, self.robots[0].ee_force[0], 
                        c='red', s=2, label='Fx')
            plt.scatter(self.timestep, self.robots[0].ee_force[1], 
                        c='green', s=2, label='Fy')
            plt.subplot(312)
            plt.scatter(self.timestep, self.robots[0].ee_force[2], 
                        c='blue', s=2, label='Fz')
            plt.subplot(313)
            plt.scatter(self.timestep, self.robots[0].ee_torque[0], 
                        c='red', s=2, label='Tx')
            plt.scatter(self.timestep, self.robots[0].ee_torque[1], 
                        c='green', s=2, label='Ty')
            plt.scatter(self.timestep, self.robots[0].ee_torque[2], 
                        c='blue', s=2, label='Tz')
            #Adjust X-limits based on timestep
            if self.timestep==1:
                plt.subplot(311)
                plt.legend()
                plt.subplot(312)
                ...
                plt.figure(1)
                plt.legend()
            elif self.timestep==int(self.horizon/4):
                plt.subplot(311)
                plt.axis([self.horizon/4,self.horizon/2,self.fxy_minmax[0],self.fxy_minmax[1]])
                plt.subplot(312)
                plt.axis([self.horizon/4,self.horizon/2,self.fz_minmax[0],self.fz_minmax[1]])
                plt.subplot(313)
                plt.axis([self.horizon/4,self.horizon/2,self.T_minmax[0],self.T_minmax[1]])
            elif self.timestep==int(self.horizon/2):
                plt.subplot(311)
                plt.axis([self.horizon/2,3*self.horizon/4,self.fxy_minmax[0],self.fxy_minmax[1]])
                plt.subplot(312)
                plt.axis([self.horizon/2,3*self.horizon/4,self.fz_minmax[0],self.fz_minmax[1]])
                plt.subplot(313)
                plt.axis([self.horizon/2,3*self.horizon/4,self.T_minmax[0],self.T_minmax[1]])
            elif self.timestep==int(3*self.horizon/4):
                plt.subplot(311)
                plt.axis([3*self.horizon/4,self.horizon,self.fxy_minmax[0],self.fxy_minmax[1]])
                plt.subplot(312)
                plt.axis([3*self.horizon/4,self.horizon,self.fz_minmax[0],self.fz_minmax[1]])
                plt.subplot(313)
                plt.axis([3*self.horizon/4,self.horizon,self.T_minmax[0],self.T_minmax[1]])
            plt.pause(0.001)
        ...
        if self.done:
            if self.save_obs_data:
                #Save data (t, peg_pos, force, torque)
                filename=f'{self.time_id}_episode{self.episode_num}_data.csv'
                with open(filename, mode='w', newline='') as csv_file:
                    csv_writer = csv.writer(csv_file, delimiter=',')
                    #Write headers
                    csv_writer.writerow(['t', 'peg_pos.x', 'peg_pos.y', 'peg_pos.z',
                    ...
                    while t<=self.timestep:
                        csv_writer.writerow([t,
                                             self.data_history['peg_pos'][t-1][0],
                                             self.data_history['peg_pos'][t-1][1],
                                             self.data_history['peg_pos'][t-1][2],
                                             self.data_history['hole_pos'][t-1][0],
                                             self.data_history['hole_pos'][t-1][1],
                                             self.data_history['hole_pos'][t-1][2],
                                             self.data_history['force'][t-1][0],
                                             self.data_history['force'][t-1][1],
                                             self.data_history['force'][t-1][2],
                                             self.data_history['torque'][t-1][0],
                                             self.data_history['torque'][t-1][1],
                                             self.data_history['torque'][t-1][2]])
                        t+=1
                #Reset history
                self.data_history={'peg_pos':[], 'hole_pos':[], 'force':[], 'torque':[]}
            if self.live_plot:
