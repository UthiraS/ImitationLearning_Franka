...
class InsertionTrainer:
    def __init__(self,num_episodes=500, max_t=300, ee0=[0.4, 0.0, 0.3],
        ...
        print(dir(self.env))
        print(dir(self.env.env))
        # print(self.env.env.has_renderer)
        # self.env.env.has_renderer=True

        #TODO: Normalize action space.
        #Define action space for OSC_POSE control
        self.env.action_space = spaces.Box(low=np.array([-0.05,-0.05,-0.05,0.0,0.0,0.0,0.0]), 
                                           high=np.array([0.05,0.05,0.01,0.0,0.0,0.0,0.0]))
...
if __name__=='__main__':
    it=InsertionTrainer(num_episodes=10, max_t=2000, ee0=[0.67,0.0,0.38], #(0.6375,0.0,0.45)
                        action_type='sac', control="OSC_POSE",
                        filename='1000ep_2000t_newrewardpun', #For saving weights
                        weight_basename=None, #For loading weights
