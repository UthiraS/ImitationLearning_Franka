        ...
        for figID in range(len(plots)):
            ...
            if idx == 0:
                ...
                if type == 1:
                    ...
                    cols = np.ceil(min(3,(remainingPlots+1)/2))
                else:
                    rows = min(3,remainingPlots)
                    cols = np.ceil(min(2,(remainingPlots+1)/3))
                newImg = Image.new('RGB', (int(cols)*dims[0], int(rows)*dims[1]))
            if type == 1:
                newImg.paste(plots[figID],(int((idx/2)%3*dims[0]),int(idx%2*dims[1])))
            else:
                newImg.paste(plots[figID],(int((idx/3)%2*dims[0]),int(idx%3*dims[1])))
            nDone += 1
            if idx == 4 or figID == len(plots) - 1:
...
def genSummary(fileName,nBins):
    ...
    for each in summary:
        ...
        for nSteps in each[4]:
            if nSteps != 99:
                nSteps = min(int(maxSteps+1),int(nSteps))
                each[5][nSteps] += 1
