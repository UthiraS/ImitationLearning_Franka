...
BFSObjs = []
#heuristicPolicies = ["Brick", "Random", "3D_Heuristic", "PCALDALR"]
heuristicPolicies = ["BRICK", "RANDOM", "3DHEURISTIC", "PCALDALR_GASD", "PCALDA_GASD_0.9", "PCALR_GASD_0.9", "PCALDALR_HAF", "PCALDALR_VFH", "BFS", "C_OPT"]# "PCALDA_HAF_0.9", "PCALDA_VFH_0.9", "PCALR_HAF_0.9", "PCALR_VFH_0.9"]
maxSteps = 21

graphColors =  ['b', 'g', 'r', 'c', 'm', 'y', 'k', 'slateblue', 'palegreen']
graphMarkers = ["o", "v", "^", "<", ">", "*", "+", "s", "o"]

def calcStepsDir(each):
    bin = [0]*maxSteps
    i = 14
    step = 1
    while i < len(each)-3 and step <= maxSteps and each[i+3] != '':
...
def genSummary(path,fileNames):
    ...
    for file in fileNames:
        ...
        for each in data:
            ...
            if policy in heuristicPolicies:
                if("BFS" == policy):
                    policyWise[keyPolicy][4*int(nSteps)] += 1
                else:
                    policyWise[keyPolicy][int(nSteps)] += 1
                for step,dir in zip(list(range(len(temp))),temp):
                    if dir != 0:
                        policyWise2[keyPolicy][step][dir-1] += 1
            else:
                # updateSummary(policyWise, policyWise2, obj)
                policyWise[keyPolicy][stVec][int(nSteps)] += 1
                stVecWise[keyStVec][policy][int(nSteps)] += 1
                for step,dir in zip(list(range(len(temp))),temp):
                    if dir != 0:
...
def graphSummary(path,policyWise,policyWise2,stVecWise):
    ...
    for key, value in list(stVecWise.items()):
        ...
        idx = 0
        # print key, value
        orderedValues = list(value.items())
        orderedValues.sort()
        for policy, summary in orderedValues:
