...
def build_actor(lr, input_dims, layer_size, num_actions=1, upper_bound=180.0):
    # Initialize weights between -3e-3 and 3-e3
    last_init = tf.random_uniform_initializer(minval=-0.003, maxval=0.003)

    inputs = layers.Input(shape=(input_dims,))
    out = layers.Dense(layer_size, activation="relu")(inputs)
...
class Agent(object):
	def __init__(self, alpha, gamma, n_actions, epsilon, batch_size, input_dims, epsilon_decount=0.996, 
		epsilon_min=0.01, mem_size=100000, upper_bound = 180.0, fname='q_model'):
		...
		self.q_eval = build_actor(alpha, input_dims, 256, n_actions, upper_bound)
	...
	def choose_action(self, state):
		...
		if rand < self.epsilon:
			action = [np.random.uniform(-self.upper_bound, self.upper_bound)]
		else:
	...
	def learn(self):
		...
		q_target[batch_index, 0] = reward + self.gamma*np.max(q_next, axis=1)*done
...
if __name__ == '__main__':
	...
	# nextMove = rospy.ServiceProxy('/active_vision/moveKinectF', controlSRVF)
	...
	agent = Agent(alpha=0.005, gamma=0.9, n_actions=num_actions, epsilon=1.0, batch_size=64, input_dims=num_inputs, 
		epsilon_decount=0.996, epsilon_min=0.01, mem_size=200000, upper_bound=2.0, fname='DDPG1')
	scores = []
	std_devs = []
	avg_scores = []
	eps_history = []
