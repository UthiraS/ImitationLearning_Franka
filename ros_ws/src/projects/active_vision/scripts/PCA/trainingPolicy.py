...
class Random_Model(methodPipeline):
    ...
    def predict(self, X):
        return int(np.random.choice(range(1,360), X.shape[0]))
...
class Brick_Model(methodPipeline):
    ...
    def predict(self, X):
        return int(np.random.choice(range(104,105), X.shape[0]))
        # return int(np.random.choice(range(6,7), X.shape[0]))


# PCA -> SVM Model
class PCA_SVM_Model(methodPipeline):
    def __init__(self, n_components):
        methodPipeline.__init__(self)
        self.n_components = n_components
    ...
    def train(self, X, y):
        ...
        self.svc = SVC(kernel='poly', degree = 3, C = 1, decision_function_shape = 'ovo')
        self.svc.fit(X_PCA, y)
    ...
    def predict(self, X):
        if(not self.ready):
            ...
            return self.svc.predict(self.pca.transform(self.normalize(X)))
