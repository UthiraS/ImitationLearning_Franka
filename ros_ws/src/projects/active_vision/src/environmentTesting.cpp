...
void testUpdateUnexpPtCld(environment &av, int objID, int flag){
  ...
  // Camera positions
  std::vector<std::vector<double>> camPoses;
  camPoses = {{av.viewsphereRad,-M_PI,M_PI/4},
              {av.viewsphereRad,-M_PI-M_PI/2,M_PI/4},
              {av.viewsphereRad,-M_PI+M_PI/2,M_PI/4},
...
void testGripperOpenClose(environment &av){
  ...
  do {
    ...
    if (width != -1) {
      ...
      av.exposeGripperPosClient().call(grasp);
...
void testRotAndShake(environment &av){
  ...
  av.exposeVelScalingClient().call(velscale);
  ...
  for(int i = 0; i < 2; i++){
    setJointWithTimeMsg.request.joint_angle = 0.000; av.exposeOneJointWithTimeClient().call(setJointWithTimeMsg);
    setJointWithTimeMsg.request.joint_angle = 1.570; av.exposeOneJointWithTimeClient().call(setJointWithTimeMsg);
  }
  setJointWithTimeMsg.request.joint_angle = 0.785; av.exposeOneJointWithTimeClient().call(setJointWithTimeMsg);

  std::cout << "Shaking Test" << std::endl;
  setJointWithTimeMsg.request.joint_name = "panda_joint5";
  for(int i = 0; i < 4; i++){
    setJointWithTimeMsg.request.joint_angle = -0.35; av.exposeOneJointWithTimeClient().call(setJointWithTimeMsg);
    setJointWithTimeMsg.request.joint_angle =  0.35; av.exposeOneJointWithTimeClient().call(setJointWithTimeMsg);
  }
  setJointWithTimeMsg.request.joint_angle =  0.00; av.exposeOneJointWithTimeClient().call(setJointWithTimeMsg);
