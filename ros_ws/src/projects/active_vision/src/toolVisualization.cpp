...
void addRGBN(ptCldVis::Ptr viewer, ptCldColor::ConstPtr cloud, ptCldNormal::ConstPtr normal, std::string name,int vp){
  ...
  viewer->addPointCloud<pcl::PointXYZRGB>(cloud,rgb,name,vp);
  viewer->setPointCloudRenderingProperties(pcl::visualization::PCL_VISUALIZER_POINT_SIZE,3,name,vp);
  viewer->addPointCloudNormals<pcl::PointXYZRGB, pcl::PointNormal>(cloud,normal,1,0.01,name+"_normal",vp);
}
...
void showVoxelBounds(ptCldVis::Ptr viewer, pcl::octree::OctreePointCloud<pcl::PointXYZRGB>::Ptr oct, int vp){
  double x1, y1, z1, x2, y2, z2, res;
  oct->getBoundingBox(x1, y1, z1, x2, y2, z2);
  res = oct->getResolution()/2.0;
  viewer->addCube(x1, x2, y1, y2, z1, z2, 1.0, 1.0, 1.0, "Bounds", vp);
  std::vector<pcl::PointXYZRGB, Eigen::aligned_allocator<pcl::PointXYZRGB>> centers;
  oct->getOccupiedVoxelCenters(centers);
  for(pcl::PointXYZRGB c : centers){
    x1 = c.x - res;
    x2 = c.x + res;
    y1 = c.y - res;
    y2 = c.y + res;
    z1 = c.z - res;
    z2 = c.z + res;
    std::string n = std::to_string(x1) + " " + std::to_string(y1) + " " + std::to_string(z1);
    viewer->addCube(x1, x2, y1, y2, z1, z2, 0.0, 1.0, 0.0, n.c_str(), vp);
  }
}
