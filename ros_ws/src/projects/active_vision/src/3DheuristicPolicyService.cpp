...
bool objCheckIfOccluded(pcl::PointXYZRGB &home, pcl::PointXYZRGB &check, pcl::PointNormal &normal, float radius){
...
void extractUsefulUnexpPts(ptCldColor::Ptr obj, ptCldColor::Ptr unexp, ptCldColor::Ptr res){
  ...
  pcl::NormalEstimation<pcl::PointXYZRGB, pcl::PointNormal> ne;  // Normal Estimation
...
int findDirection(ptCldColor::Ptr obj, ptCldColor::Ptr unexp, std::vector<double> &pose, int mode, int minVis, active_vision::heuristicPolicySRV::Response &response){
  ...
  response.vecVisiblePointsAllDirection.data.clear();
  for(int i = 0; i < 8; i++)
  {
      response.vecVisiblePointsAllDirection.data.push_back(res[i]);
  }
...
bool heuristicPolicy(active_vision::heuristicPolicySRV::Request  &req,
  ...
  res.direction = findDirection(obj, unexp, path.back(), mode, req.minPtsVis, res)-1;
