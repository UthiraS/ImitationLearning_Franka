...
def pivot(theta_finger,dx,theta_pivot,dz,dl):
    print("test_pivoting",theta_finger,dx,theta_pivot,dz,dl)
    # get ee pose: through gazebo/get_link_state, link name: panda_link7
    # rospy.wait_for_service('gazebo/get_link_state')
    # try:
    #     gls = rospy.ServiceProxy('gazebo/get_link_state', GetLinkState)
...
def execute_plan():
    ...
    # # If planning outputs are saved at home:
    # home_dir = os.path.expanduser("~")
    # output_folder = home_dir + "/reg_based_plans"

    # If planning outputs are saved within region_based_planning:
    dir_path = os.path.dirname(os.path.realpath(__file__))
    output_folder = dir_path + "/../output_plans/"

    obj_type = "square_prism"
    exp_no = 1
    date = "2021-05-11_23-40"
    file_path = "/plan_" + obj_type + "_" + str(exp_no) + "_" + date
    ...
    for step in range(0,len(action_list)):
        ...
        if action=="'Slide left down'":
            ...
            dx = command_inst[step][1]/100
            theta_pivot = command_inst[step][2]
            dz = command_inst[step][3]/100
            dl = command_inst[step][4]/100
            print("Pivoting")
            pivot(theta_finger,dx,theta_pivot,dz,dl)
        else:
