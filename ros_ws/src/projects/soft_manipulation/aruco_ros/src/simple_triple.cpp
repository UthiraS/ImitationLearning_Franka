...
//        inImage = inImageNorm;
      ...
      if (markers.size() == 3)
      {
        ...
        ROS_DEBUG_STREAM(
            "Mid point between the three markers in the image = (" << (x[0]+x[1]+x[2])/3 << ", " << (y[0]+y[1]+y[2])/3 << ")");
...
//        cv::circle(inImage, cv::Point((u[0] + u[1]) / 2, (v[0] + v[1]) / 2), 3, cv::Scalar(0, 0, 255), cv::FILLED);
        ...
        for (unsigned int i = 0; i < 3; ++i)
          midPoint3D[i] = (markers[0].Tvec.at<float>(i, 0) + markers[1].Tvec.at<float>(i, 0) + markers[2].Tvec.at<float>(i,0)) / 3;
...
int main(int argc, char **argv)
{
  ...
  ROS_INFO("ArUco node started with marker size of %f meters and marker ids to track: %d, %d, %d", marker_size, marker_id1,
           marker_id2, marker_id3);
  ROS_INFO("ArUco node will publish pose to TF with (%s, %s) and (%s, %s) and (%s, %s) as (parent,child).", parent_name.c_str(),
           child_name1.c_str(), parent_name.c_str(), child_name2.c_str(), parent_name.c_str(), child_name3.c_str());
